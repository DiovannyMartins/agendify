...
describe("lookupBookingByPublicCode", () => {
  it("maps a found row to a normalized public booking", async () => {
    const res = await lookupBookingByPublicCode(fetcher, "65925dbb-ab9d-42eb-832a-030c1b28d1e4");
    expect(res).toEqual({
      ok: true,
      data: {
        serviceName: "Corte",
  ...
  it("returns NOT_FOUND when no booking matches the code", async () => {
    ...
    const res = await lookupBookingByPublicCode(empty, "65925dbb-ab9d-42eb-832a-030c1b28d1e4");
    expect(res).toMatchObject({ ok: false, code: "NOT_FOUND" });
  });

  it("returns INVALID_CODE for a non-uuid code without calling the fetcher", async () => {
    let called = false;
    const spy: LookupFetcher = async () => {
      called = true;
      return { data: row, error: null };
    };
    const res = await lookupBookingByPublicCode(spy, "not-a-uuid");
  ...
  it("returns DB_ERROR when the database reports an error", async () => {
    ...
    const res = await lookupBookingByPublicCode(failing, "65925dbb-ab9d-42eb-832a-030c1b28d1e4");
